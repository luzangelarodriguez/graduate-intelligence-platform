...
CREATE TABLE IF NOT EXISTS public.ml_evaluation_runs (
    ...
    precision_macro NUMERIC(5, 4) NOT NULL DEFAULT 0,
    recall_macro NUMERIC(5, 4) NOT NULL DEFAULT 0,
    f1_macro NUMERIC(5, 4) NOT NULL DEFAULT 0,
    confusion_matrix JSONB,
    metrics JSONB,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS ix_ml_predictions_domain_confidence
ON public.ml_predictions(predicted_domain, confidence DESC);

CREATE INDEX IF NOT EXISTS ix_ml_evaluation_runs_created
ON public.ml_evaluation_runs(created_at DESC);

CREATE TABLE IF NOT EXISTS public.microcurriculo_processing_runs (
    run_id TEXT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS public.microcurriculos (
    id BIGSERIAL PRIMARY KEY,
    run_id TEXT REFERENCES public.microcurriculo_processing_runs(run_id) ON DELETE SET NULL,
    programa TEXT,
    asignatura TEXT,
    semestre TEXT,
    creditos TEXT,
...
CREATE TABLE IF NOT EXISTS public.microcurriculo_asignaturas (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS public.microcurriculo_skills (
    id BIGSERIAL PRIMARY KEY,
    microcurriculo_id BIGINT NOT NULL REFERENCES public.microcurriculos(id) ON DELETE CASCADE,
    skill_original TEXT,
    skill_normalized TEXT NOT NULL,
    ...
    lineage JSONB,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE (microcurriculo_id, skill_normalized, tipo_skill)
);

CREATE TABLE IF NOT EXISTS public.microcurriculo_competencias (
    id BIGSERIAL PRIMARY KEY,
    microcurriculo_id BIGINT NOT NULL REFERENCES public.microcurriculos(id) ON DELETE CASCADE,
    competencia_text TEXT NOT NULL,
    competencia_type TEXT NOT NULL DEFAULT 'competencia',
    confidence_score NUMERIC(5, 4) NOT NULL DEFAULT 0,
    source_document TEXT,
    lineage JSONB,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
...
CREATE TABLE IF NOT EXISTS public.microcurriculo_plataformas (
    id BIGSERIAL PRIMARY KEY,
    microcurriculo_id BIGINT NOT NULL REFERENCES public.microcurriculos(id) ON DELETE CASCADE,
    plataforma TEXT NOT NULL,
    plataforma_normalized TEXT,
    confidence_score NUMERIC(5, 4) NOT NULL DEFAULT 0,
    source_document TEXT,
    lineage JSONB,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE (microcurriculo_id, plataforma_normalized)
);

CREATE TABLE IF NOT EXISTS public.microcurriculo_herramientas (
    id BIGSERIAL PRIMARY KEY,
    microcurriculo_id BIGINT NOT NULL REFERENCES public.microcurriculos(id) ON DELETE CASCADE,
    herramienta TEXT NOT NULL,
    herramienta_normalized TEXT,
    tipo TEXT,
    confidence_score NUMERIC(5, 4) NOT NULL DEFAULT 0,
    source_document TEXT,
    lineage JSONB,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE (microcurriculo_id, herramienta_normalized)
);

CREATE TABLE IF NOT EXISTS public.microcurriculo_embeddings (
    id BIGSERIAL PRIMARY KEY,
    microcurriculo_id BIGINT NOT NULL REFERENCES public.microcurriculos(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.microcurriculo_market_gaps (
    id BIGSERIAL PRIMARY KEY,
    microcurriculo_id BIGINT NOT NULL REFERENCES public.microcurriculos(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.microcurriculo_recommendations (
    id BIGSERIAL PRIMARY KEY,
    microcurriculo_id BIGINT NOT NULL REFERENCES public.microcurriculos(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS ix_microcurriculos_domain
ON public.microcurriculos(detected_domain, domain_confidence DESC);

CREATE INDEX IF NOT EXISTS ix_microcurriculo_skills_skill
ON public.microcurriculo_skills(skill_normalized);

CREATE INDEX IF NOT EXISTS ix_microcurriculo_gaps_skill
ON public.microcurriculo_market_gaps(skill_normalized, severity);

CREATE INDEX IF NOT EXISTS ix_microcurriculo_recommendations_micro
ON public.microcurriculo_recommendations(microcurriculo_id, confidence_score DESC);
