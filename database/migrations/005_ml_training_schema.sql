...
BEGIN
    IF to_regclass('public.empleos') IS NOT NULL THEN
        IF EXISTS (
            ...
            WHERE table_schema = 'public' AND table_name = 'empleos' AND column_name = 'location'
        ) THEN
            EXECUTE 'UPDATE public.empleos SET ubicacion = COALESCE(NULLIF(ubicacion, ''''), location) WHERE ubicacion IS NULL OR ubicacion = ''''';
        END IF;

        IF EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = 'public' AND table_name = 'empleos' AND column_name = 'source'
        ) THEN
            EXECUTE 'UPDATE public.empleos SET fuente = COALESCE(NULLIF(fuente, ''''), source) WHERE fuente IS NULL OR fuente = ''''';
        END IF;

        IF EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = 'public' AND table_name = 'empleos' AND column_name = 'job_url'
        ) THEN
            EXECUTE 'UPDATE public.empleos SET url = COALESCE(NULLIF(url, ''''), job_url) WHERE url IS NULL OR url = ''''';
        END IF;
    END IF;
END $$;
