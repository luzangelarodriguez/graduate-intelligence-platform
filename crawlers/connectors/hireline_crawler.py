...
class HirelineCrawler(StructuredConnectorCrawler):
    def __init__(self, *, max_jobs: int = 20, max_pages: int = 2, search_intelligence: dict | None = None) -> None:
        ...
        plan = source_plan_for(intelligence.get('crawler_plans'), 'hireline')
        super().__init__(HirelineConnector(max_jobs=max_jobs, max_pages=max_pages, source_plan=plan), 'hireline', source_plan=plan)
